...
RC10 is a native TFT presentation refinement for the WS350. It preserves RC9
interaction geometry, network workflow semantics, guarded printer actions,
authentication, recovery, browser command authorization, and inventory authority
boundaries. The change replaces the primary native visual language with a calmer
Cupertino-inspired dark interface using grouped surfaces, system-like blue
accenting, segmented controls, and a tab-bar treatment.
"""
...
def sections(source_root: Path) -> dict[str, str]:
    values: dict[str, str] = {}
    fragment_dir = source_root / "firmware" / "ui-v11.25-rc10"
    ...
    if missing:
        raise PatchError(f"missing RC10 fragments: {sorted(missing)}")
    return values
...
def patch(repo: Path) -> None:
    ...
    build = once(
        ...
        '#define SMART_HOME_BUILD_LABEL "Workshop OS v11.25 RC10 Cupertino UI"',
        "RC10 build label",
    )
    build = once(
        build,
        '#define SMART_HOME_PROFILE "premium-appliance-ui"',
        '#define SMART_HOME_PROFILE "cupertino-ui"',
        "RC10 profile",
    )
    build = once(
        build,
        '#define WORKSHOP_OS_UI_SCHEMA "UI9"',
        '#define WORKSHOP_OS_UI_SCHEMA "UI10"',
        "RC10 UI schema",
    )
    if "WORKSHOP_OS_V11_25_UI_RC10" not in build:
    ...
    if hub.count("securityPortalCode()") < 2:
        raise PatchError("RC10 lost secure portal-code visibility")
    if "TEST / NO CODE" in hub:
        raise PatchError("RC10 reintroduced insecure no-code copy")
    ...
    for marker in (
        ...
        if marker not in hub:
            raise PatchError(f"missing RC10 UX marker: {marker}")

    # RC8/RC9 physical geometry and guarded interaction contract must stay intact.
    for anchor in (
        "static int16_t hubHeaderH() { return 36; }",
        "static int16_t hubNavH() { return 54; }",
        "static const uint8_t HUB_NETWORK_PAGE_COUNT = 7;",
        "Keep Holding",
        "Hold to Apply",
    ):
        if anchor not in hub:
            raise PatchError(f"RC10 lost inherited interaction anchor: {anchor}")
