...
def dedupe_action_button(repo: Path) -> None:
    ...
    while True:
        ...
        pos=i+len(signature)
    if len(starts)!=2:
        raise PatchError(f"uiActionButton: expected 2 definitions before v10 fixup, found {len(starts)}")
    start=starts[1]
...
def main() -> int:
    ...
    print("Smart Home v10 Workshop OS Theme applied (compile fixup included)")
    return 0
