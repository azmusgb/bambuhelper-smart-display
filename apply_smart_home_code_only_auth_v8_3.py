...
def patch_web_pages(repo: Path) -> None:
    ...
    # Keep the old password controls in the DOM (hidden/disabled) so shared UI
    # code remains backward-compatible, but there is no user path to select them.
    text = replace_once(
        text,
    ...
    # v8 already disables persistence. v8.3 hides the password entry surface
    # entirely while retaining the nodes for defensive compatibility with old JS.
    text = re.sub(
...
'''
    ...
    # Initialize the sign-in pane explicitly in code-only mode.
    text = replace_once(
        text,
    ...
    # The password branch remains unreachable, but remove it anyway so browser
    # code can never serialize a Bambu account password into a request body.
