#!/usr/bin/env python3
"""Apply Workshop OS v11.25 RC7 touch-first UX overhaul.

RC7 is intentionally presentation-only. It preserves printer command semantics,
network/restart guards, recovery, authentication, persistence, and device I/O.
It replaces the RC5 native presentation and performs conservative copy/layout
normalization across the legacy settings pages generated earlier in the chain.
"""
...
def sections(source_root: Path) -> dict[str, str]:
    ...
    required = {
        "FINGERPRINT_HELPERS", "CARD", "MODE_TABS", "ACTION", "TELEMETRY",
        "AMS_SLOT", "HEADER", "BOTTOM_NAV", "HOME", "PRINTER", "WORKSHOP",
        "MORE", "SYSTEM",
    }
    missing = required - set(values)
    if missing:
        raise PatchError(f"missing RC7 fragments: {sorted(missing)}")
    return values
...
def normalize_settings_copy(hub: str) -> str:
    """Normalize legacy settings copy without changing handlers or state semantics."""
    replacements = (
        ('"< BACK"', '"BACK"'),
        ('"< PREV"', '"PREV"'),
        ('"NEXT >"', '"NEXT"'),
        ...
        ('"STAGED CONFIGURATION - REVIEW BEFORE APPLY"', '"REVIEW CHANGES"'),
        ('"STAGED - NOT APPLIED"', '"CHANGES NOT APPLIED"'),
        ('"Use explicit PREV / NEXT"', '"Choose timezone"'),
        ('"Tap to toggle"', '"Tap to change"'),
        ('"Tap to cycle"', '"Tap to change"'),
        ('"Hostname remains browser-only"', '"Local network name"'),
        ('"Timezone changes apply immediately. Wi-Fi credentials stay browser-only."', '"Timezone saves immediately. Network changes are reviewed before apply."'),
        ('"Speaker · mic · events"', '"Speaker, microphone and alerts"'),
        ('"Screen · standby"', '"Screen and standby"'),
        ('"Health · network"', '"Network and device"'),
        ('"Timers · utilities"', '"Timers and utilities"'),
    )
    for old, new in replacements:
        hub = replace_if_present(hub, old, new)

    # Short, deliberate context names fit the 88px header chip at native size.
    context_replacements = (
        ('"DISPLAY - QUICK"', '"QUICK"'),
        ('"DISPLAY - SCHEDULE"', '"SCHEDULE"'),
        ('"DISPLAY - BEHAVIOR"', '"BEHAVIOR"'),
        ('"DISPLAY - VISUAL"', '"VISUAL"'),
        ('"DISPLAY - CLOCK"', '"CLOCK"'),
        ('"DISPLAY - ALERTS"', '"ALERTS"'),
        ('"DISPLAY - SIGNALS"', '"SIGNALS"'),
        ('"DISPLAY - THEME"', '"THEME"'),
        ('"DISPLAY - GAUGE COLORS"', '"COLORS"'),
        ('"DISPLAY - GAUGE SCALES"', '"SCALES"'),
        ('"DISPLAY - GAUGE BEHAVIOR"', '"GAUGE"'),
        ('"DISPLAY - GLOW"', '"GLOW"'),
        ('"DISPLAY - LAYOUT"', '"LAYOUT"'),
        ('"DISPLAY - EXTRAS"', '"EXTRAS"'),
        ('"NETWORK - TIME & LOCALE"', '"TIME"'),
        ('"NETWORK - ADDRESS EDITOR"', '"ADDRESS"'),
        ('"NETWORK - ADDRESS REVIEW"', '"REVIEW"'),
        ('"HARDWARE - SOUND"', '"SOUND"'),
        ('"HARDWARE - COOLDOWN"', '"COOLDOWN"'),
        ('"HARDWARE - LED"', '"LED"'),
        ('"HARDWARE - FINISH"', '"FINISH"'),
        ('"HARDWARE - ERROR"', '"ERROR"'),
        ('"HARDWARE - POWER"', '"POWER"'),
        ('"HARDWARE - AUTO OFF"', '"AUTO OFF"'),
    )
...
def patch(repo: Path) -> None:
    ...
    build = once(
        ...
        '#define SMART_HOME_BUILD_LABEL "Workshop OS v11.25 RC7 Touch-First UX Overhaul"',
        "RC7 build label",
    )
    build = replace_if_present(build, '#define SMART_HOME_PROFILE "native-ui-acceptance"', '#define SMART_HOME_PROFILE "touch-first-ux"')
    if "WORKSHOP_OS_V11_25_UI_RC7" not in build:
        build += "\n#define WORKSHOP_OS_V11_25_UI_RC7 1\n"
    if '#define WORKSHOP_OS_UI_SCHEMA "UI5"' in build:
        build = build.replace('#define WORKSHOP_OS_UI_SCHEMA "UI5"', '#define WORKSHOP_OS_UI_SCHEMA "UI7"')
    elif "WORKSHOP_OS_UI_SCHEMA" not in build:
        build += '#define WORKSHOP_OS_UI_SCHEMA "UI7"\n'
