...
DISPLAY_FOUR_PAGE_CODE = r'''static void drawDisplayExperience(bool full) {
  (void)full;
  const int16_t W = tft.width();
  const uint8_t page = (uint8_t)(g_displayExperiencePage % 4U);
  const bool quick = page == 0;
  const bool schedule = page == 1;
  const bool behavior = page == 2;
  tft.fillScreen(UI_BG);
  drawHeader("DISPLAY",
             quick ? "QUICK" : (schedule ? "SCHEDULE" : (behavior ? "BEHAVIOR" : "VISUAL")),
             3);
  ...
  if (hubLandscape()) {
    ...
    uiDrawFit(quick ? "DISPLAY & EXPERIENCE"
                    : (schedule ? "SCHEDULE & FINISH"
                                : (behavior ? "FINISH BEHAVIOR" : "VISUAL PRESENTATION")),
...
def patch(root: Path) -> None:
    ...
    t = load(root, rel)
    t = once(t, '#define SMART_HOME_VERSION "v11.10"',
             '#define SMART_HOME_VERSION "v11.11"', "version")
    t = once(t, '#define SMART_HOME_PROFILE "display-behavior"',
             '#define SMART_HOME_PROFILE "display-visual"', "profile")
    t = once(t, 'Smart Home v11.10 Display Behavior RC1',
             'Smart Home v11.11 Display Visual RC1', "label")
    save(root, rel, t)
    ...
    t = replace_between(t,
                        ...
                        DISPLAY_FOUR_PAGE_CODE,
                        "four-page Display renderer")
...
'''
    t = once(t, old_touch, new_touch, "Visual touch page")
    save(root, rel, t)

    checks = {
        "include/smart_home_build.h": [
            'SMART_HOME_VERSION "v11.11"',
            'SMART_HOME_PROFILE "display-visual"',
            'Smart Home v11.11 Display Visual RC1',
        ],
        "src/smart_hub.cpp": [
            '"VISUAL"',
            '"ANIMATED PROGRESS"',
            ...
            '"STATUS READOUT"',
            '"VISUAL >"',
            "dispSettings.animatedBar=!dispSettings.animatedBar",
            "dispSettings.smallLabels=!dispSettings.smallLabels",
            "dispSettings.fanMatchPrinter=!dispSettings.fanMatchPrinter",
            "dispSettings.hideStatusReadout=!dispSettings.hideStatusReadout",
            "(g_displayExperiencePage+1U)%4U",
        ],
    }
...
def main() -> int:
    ...
    print("Smart Home v11.11 Display Visual applied")
    return 0
