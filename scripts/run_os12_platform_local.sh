...
# UI status surfaces consume normalized read-only facts. Physical Light and
# Pause/Resume now dispatch through the OS12 command facade, which still uses
# the existing deferred Bambu MQTT transport. Guarded STOP deliberately remains
# on its legacy long-press path for a separate migration gate.
...
grep -q 'requestPrinterControlCommand(slot,PRINTER_CTRL_STOP)' "$BUILD/src/smart_hub.cpp"
...
echo "Non-destructive physical commands: OS12 facade -> existing deferred Bambu transport."
echo "Guarded STOP: intentionally remains on legacy long-press path."
