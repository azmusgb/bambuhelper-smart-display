...
grep -q 'speakerTestEndsAtMs_ = nowMs + 1200U' "$BUILD/src/media_service.cpp"
...
grep -q 'drawOs12Recorder' "$BUILD/src/smart_hub.cpp"
...
! grep -q '#define WORKSHOP_OS_TEMP_NO_CODE_LAN' "$BUILD/include/smart_home_build.h"
! grep -q '#if defined(WORKSHOP_OS_TEMP_NO_CODE_LAN) && WORKSHOP_OS_TEMP_NO_CODE_LAN' "$BUILD/src/security_manager.cpp"
! grep -q 'if (!isAPMode()) return true;' "$BUILD/src/security_manager.cpp"
