...
if [[ "${1:-}" == "--build" ]]; then
  ...
  # One repository-owned PlatformIO authority is used by UI13, OS12 platform,
  # OS12 UX, and the physical USB bootstrap. Do not fall back to a global pio:
  # host Python/package drift would make locally built bytes non-reproducible.
